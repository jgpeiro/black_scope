...
void scope_config_horizontal(tScope *pThis, int offset, int scale) {
    ...
    pThis->horizontal.htim_clock->Init.Prescaler = (SystemCoreClock / (scale)) / 2 - 1;
    pThis->horizontal.htim_clock->Init.Period = scale22*2 - 1;
    HAL_TIM_Base_Init(pThis->horizontal.htim_clock);

    // Configure the timer for stop time calculation
    pThis->horizontal.htim_stop->Init.Prescaler = (SystemCoreClock / (scale)) / 2 - 1;
    pThis->horizontal.htim_stop->Init.Period = scale22*(pThis->len/2 + offset)*2 - 1;
    HAL_TIM_Base_Init(pThis->horizontal.htim_stop);

    // Configure the timer output compare channel for triggering
    TIM_OC_InitTypeDef sConfigOC = {0};
    sConfigOC.OCMode = TIM_OCMODE_TIMING;
    sConfigOC.Pulse = scale22*(pThis->len/2 + offset)*2 - 1;
...
void scope_stop(tScope* pThis) {
	...
	if( pThis->trigged == 0 )
	{
		pThis->dma_cndtr = DMA1_Channel1->CNDTR;
	}
