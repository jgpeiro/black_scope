...
{
	if ( width < 1 || x + width <= 0 ||  y < 0 || y >= pThis->height || x >= pThis->width) {
		return;
	}
...
{
	if (height < 1 || y + height <= 0 || y >= pThis->height || x < 0 || x >= pThis->width) {
		return;
	}
    uint16_t *buf = pThis->buf + y*pThis->width + x;
...
void framebuf_circle( const tFramebuf *fb, int16_t x, int16_t y, uint16_t radius, uint16_t filled, uint8_t quadrant, uint16_t color )
{
	...
	framebuf_hline( fb, x-radius, y, 2*radius, color );
